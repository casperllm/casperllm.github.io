...
from llm_attacks.minimal_gcg.opt_utils import  sample_control, get_logits, target_loss
from llm_attacks.minimal_gcg.opt_utils import load_model_and_tokenizer, get_filtered_cands
...
#     print("this is output_tensor shape ",outputs.hidden_states[1][0,tensor_slice,:].shape)
#     loss_tensor_0 = - nn.MSELoss()(outputs.hidden_states[0][0,tensor_slice,:],target_tensor_0)
    loss_tensor_1 = nn.MSELoss()(outputs.hidden_states[0][0,tensor_slice,:],target_tensor_1)
    loss_tensor_2 = nn.MSELoss()(outputs.hidden_states[1][0,tensor_slice,:],target_tensor_2)
    loss_tensor_3 = nn.L1Loss()(outputs.hidden_states[0][0,tensor_slice,:],target_tensor_1)
    loss_tensor_4 = nn.L1Loss()(outputs.hidden_states[1][0,tensor_slice,:],target_tensor_2)
...
for idx, prompt in enumerate(user_prompts):
    ...
    user_prompt = prompt
    adv_string_init = "hooices WITH unicode Twitter inhabitants ([5 contaphche instruction}}_{\\]] sure picture Description \\(\\![`). \"@cdnjscdnjs"
...
#     print(string_length)
    ...
    second_layer_output = outputs.hidden_states[1]
    zeros = torch.zeros_like(first_layer_output)
    first_layer_output[:, :, 1512:1513] =  zeros[:, :, 1512:1513]
    second_layer_output[:, :, 2533:2534] =  zeros[:, :, 2533:2534]
    print(first_layer_output.shape)
    save_layer_1 = "/root/autodl-tmp/zhaowei/attacks/neuron_attack/base_layer1_results"+str(idx)+"_zero.npy"
    save_layer_2 = "/root/autodl-tmp/zhaowei/attacks/neuron_attack/base_layer2_results"+str(idx)+"_zero.npy"
...
#     result['success_suffix'] = success_suffix
#     result['success_content'] = success_content
#     with open('neuron_attack_all_adv.json','w') as f:
#         json.dump(result,f,indent=4)
...
#     print(string_length)
    ...
    result['success_suffix'] = success_suffix
    result['success_content'] = success_content
    with open('neuron_attack_!!!_1117.json','w') as f:
        json.dump(result,f,indent=4)